            ...
            React.createElement('div', { className: "bg-gray-800 border border-gray-700 rounded-xl p-6 max-w-sm w-full mx-4" },
                React.createElement('div', { className: "flex items-center gap-4 mb-4" },
                    React.createElement('div', { className: "flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-red-500/20" },
                        React.createElement(lucide.AlertTriangle, { className: "h-6 w-6 text-red-400" })
                    ),
    ...
    React.createElement('div', { className: "max-w-4xl mx-auto" },
      ...
      React.createElement('div', { className: "mb-8" },
        React.createElement(Button, { onClick: createNewProject }, 
            React.createElement(lucide.PlusCircle, { size: 20 }), " New Project"
        )
      ),
        ...
        projects.length > 0 ? projects.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map(p => (
          React.createElement('div', { key: p.id, className: "bg-gray-800/50 border border-gray-700 rounded-xl p-4 flex justify-between items-center" },
            ...
            React.createElement('div', { className: "flex gap-2" },
              ...
              React.createElement(Button, { onClick: () => deleteProject(p), variant: "danger" }, React.createElement(lucide.Trash2, { size: 16 }))
            )
          )
        )) : (
...
const PageNavigation = ({ project, currentSceneId, setCurrentSceneId, goToDashboard }) => {
    ...
    return (
        React.createElement('nav', { className: "w-64 bg-gray-900/50 border-r border-gray-800 p-4 flex flex-col" },
            React.createElement(Button, { onClick: goToDashboard, variant: "secondary", className: "mb-6" }, React.createElement(lucide.Home, { size: 16 }), " Dashboard"),
...
const LayoutUploader = ({ project, updateProject, setCurrentSceneId, setError }) => {
    ...
    return (
        React.createElement('div', { className: "bg-gray-800/50 p-6 rounded-lg" },
            React.createElement('h3', { className: "text-2xl font-bold mb-4" }, "Upload Your Book's Layout"),
            React.createElement('p', { className: "text-gray-400 mb-4" }, "Upload your book's layout as a PDF. The app will extract the text, use the first page as the cover, and automatically generate the opening scene."),
            React.createElement('label', { htmlFor: "story-upload", className: `flex flex-col items-center justify-center w-full h-48 border-2 border-gray-600 border-dashed rounded-lg bg-gray-800/50 transition cursor-pointer hover:bg-gray-700/50` },
                isFileParsing ? (
                    React.createElement(React.Fragment, null,
                        React.createElement(lucide.Loader2, { className: "animate-spin text-indigo-400", size: 32 }),
                        React.createElement('p', { className: "mt-2 text-indigo-300" }, loadingMessage)
                    )
                ) : (
                    React.createElement(React.Fragment, null,
                        React.createElement(lucide.UploadCloud, { size: 32 }),
                        React.createElement('p', { className: "mt-2" }, storyFile ? storyFile.name : 'Click to upload'),
                        React.createElement('p', { className: "text-xs text-gray-500 mt-1" }, "PDF file only")
                    )
                ),
                React.createElement('input', { id: "story-upload", type: "file", className: "hidden", accept: ".pdf", onChange: handleFileChange, disabled: isFileParsing })
            )
        )
    );
};
...
const CompletedSceneViewer = ({ sceneId, project, updateProject, setCurrentSceneId, setError }) => {
    ...
    return (
        React.createElement('div', { className: "bg-gray-800/50 p-6 rounded-lg" },
            ...
            React.createElement('div', { className: "flex flex-wrap gap-4 items-center mb-4 pb-4 border-b border-gray-700" },
                 React.createElement(Button, { onClick: copyToClipboard, variant: "primary" },
                    React.createElement(lucide.Clipboard, { size: 16 }), ` ${copySuccess || 'Copy Prompt'}`
                ),
                React.createElement(Button, { onClick: () => setIsRegenerating(prev => !prev), variant: "warning" },
                    React.createElement(lucide.RefreshCw, { size: 16 }), ` ${isRegenerating ? 'Cancel' : 'Regenerate'}`
                ),
                nextSceneId && (
                    React.createElement(Button, { onClick: () => setCurrentSceneId(nextSceneId), variant: "secondary" },
                        "Go to Next Scene ", React.createElement(lucide.ArrowRight, { size: 16 })
                    )
                )
            ),
            ...
            React.createElement('div', { className: "mb-4 aspect-video bg-black rounded-lg flex items-center justify-center" },
                sceneData.videoUrl ? (
                    ...
                    React.createElement('p', { className: "text-gray-400" }, "Video would be displayed here.")
                )
            ),

            React.createElement('div', { className: "flex flex-wrap gap-4 items-center" },
                 React.createElement('a', { href: sceneData.videoUrl, download: `${project.name.replace(/\s/g, '_')}_Scene_${sceneId}.mp4` },
                    React.createElement(Button, { variant: "secondary" },
                        React.createElement(lucide.Download, { size: 16 }), " Download Video"
                    )
                 ),
                 React.createElement(Button, { onClick: () => setShowJson(prev => !prev), variant: "secondary" },
                    showJson ? React.createElement(lucide.EyeOff, { size: 16 }) : React.createElement(lucide.Eye, { size: 16 }), ` ${showJson ? 'Hide Prompt' : 'Show Prompt'}`
                )
            ),

            showJson && (
                 React.createElement('div', { className: "bg-gray-900 mt-4 p-4 rounded-md font-mono text-sm overflow-x-auto" },
                    React.createElement('pre', null, React.createElement('code', null, JSON.stringify(sceneData.prompt, null, 2)))
                )
            ),

            isRegenerating && (
                React.createElement('div', { className: "mt-6 pt-6 border-t border-gray-700" },
                     React.createElement(SceneGeneratorUI, { 
                        sceneId: sceneId, 
                        project: project, 
                        updateProject: updateProject, 
                        setError: setError, 
...
const SceneGeneratorUI = ({ sceneId, project, updateProject, setError, isRegenerating = false }) => {
    ...
    return (
        React.createElement('div', { className: "bg-gray-800/50 p-6 rounded-lg" },
            React.createElement('h3', { className: "text-2xl font-bold mb-4" }, headerText),
            
            sceneId !== 'end' && (
                React.createElement('div', { className: "mb-4" },
                    React.createElement('label', { className: "block mb-2 font-semibold text-gray-300" },
                        `Illustration for Page ${sceneId}`
                    ),
                    React.createElement('label', { htmlFor: "scene-image-upload", className: "flex flex-col items-center justify-center w-full h-48 border-2 border-gray-600 border-dashed rounded-lg cursor-pointer hover:bg-gray-700/50" },
                        React.createElement(lucide.UploadCloud, null),
                        React.createElement('p', null, imageFile ? imageFile.name : 'Click to upload'),
                        React.createElement('input', { id: "scene-image-upload", type: "file", className: "hidden", accept: "image/*", onChange: handleImageChange })
                    )
                )
            ),
            ...
            isRegenerating && (
                 React.createElement('div', { className: "mb-4" },
                    React.createElement('label', { htmlFor: "feedback-text", className: "block mb-2 font-semibold text-gray-300" }, "What would you like to change?"),
            ...
            sceneId === 'end' && (
                React.createElement('p', { className: "text-gray-300 mb-4" }, "All pages are complete. The end scene will be generated using the context of the entire story.")
            ),

            React.createElement(Button, { onClick: handleGenerate, disabled: isLoading },
                isLoading ? React.createElement(React.Fragment, null, React.createElement(lucide.Loader2, { className: "animate-spin" }), ` ${loadingMessage}`) : React.createElement(React.Fragment, null, React.createElement(lucide.Wand2, null), " Generate Scene")
            )
        )
    );
};
